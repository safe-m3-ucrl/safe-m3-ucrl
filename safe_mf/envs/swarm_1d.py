...
class Swarm1D(Env):
    def __init__(
        ...
        action_space: Tuple[float, float] = (-7.0, 7.0),
        ...
        with torch.no_grad():
            ...
            self.extended_cell_centers_1d = (
                torch.arange(-self.num_intervals, 2 * self.num_intervals, 1, device=self.device)
                .reshape(1, -1)
                / self.num_intervals
            ) + 0.5 / self.num_intervals

        self.lower = self.extended_cell_centers_1d - (0.5 / self.num_intervals)
        self.upper = self.extended_cell_centers_1d + (0.5 / self.num_intervals)
    ...
    def _true_dynamics(
        ...
        actions = policy(states.reshape(-1, 1), mu, action_std)
    ...
    def _approximate_dynamics(
        ...
        means, epi, alea = self.dynamics(
            states.reshape(-1, self.state_dim),
            mu, 
            true_actions.reshape(-1, self.action_dim)
        )
        epi = torch.diagonal(epi, dim1=1, dim2=2)
        hallucinated_actions = self.dynamics.beta * torch.sqrt( # Why do we sqrt?
            epi
        ) * hallucinated_actions.reshape(-1, self.state_dim)
        next_states = self.get_next_states(means.reshape(1, -1), hallucinated_actions.reshape(1, -1), mf_transition)

        return actions, next_states


    def _rew(self, states: torch.Tensor):
    ...
    def reward(
        ...
        if safety:
            return self._rew(states) - 0.5 * actions**2
        else:
            return (
                self._rew(states)
                - 0.5 * actions**2
                - torch.log(torch.clamp(mu, min=1e-4))
            )
    ...
    def step(
        ...
        if self.exec_type == "eval":
            ...
            next_mu = self._step_probablistic(grid_next_states, current_mu)
    ...
    def _step_probablistic(
        ...
        ERF based
        """
        probs = self._apply_erf(next_states)
